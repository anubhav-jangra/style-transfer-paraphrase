...
ALL_MODELS = sum((tuple(GPT2_PRETRAINED_CONFIG_ARCHIVE_MAP.keys()) for conf in (GPT2Config,)), ())
...
def load_and_cache_examples(args, tokenizer):
    print("prefix_input_type:", args.prefix_input_type)
...
def main():
    ...
    # check if '\n' character is there in the generated output
    generated_text = []
    for line in output_log["generated_text"]:
        if line.find('\n') != -1:
            print('***', line, '***')
            line = line.replace('\n', ' ')
